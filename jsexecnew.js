...
function Reference(base, propertyName, node) {
    ...
    p( " Reference () ");
    p( " \t \t base - " + base + " property - " + propertyName  + " node - " + node);
}
...
//getValueSymbolic(n[i], x)
function getValueSymbolic(v)
{
    p ("getValueSymbolic: ");
    p (" \t \t  + v.propertyName + r :: " + 
        (v.base||global)[notamper_getPropertyName(v.propertyName)]);
    return (v.base||global)[notamper_getPropertyName(v.propertyName)];
}


function putValue(v, w, vn) {
        p(" putValue");
        p("\t\t : " + v + " property name "+ v.propertyName + " w = " + w + " file - " + v.node.filename  + " line - " + v.node.lineno);
    ...
    if (v instanceof Reference)
    {
        ...
        p ( " \t\t\t v.base = " + v.base + " global - " + global + 
            " v.propertyName - " + v.propertyName + " w -  " + w + 
...
//    p ( " execute : called " + n + " and x - " + x );
    ...
    switch (n.type) {
      ...
      case IF:
        if ( IF == n.type )
            p(" Execute : If condition - " + n.condition + " then - " + n.thenPart + " else - " + n.elsePart + " execution context - " + x);
      ...
      case ASSIGN:
        ...
        p ( " \t\t n[1] - " + n[1]);
        p ( " \t\t u -" + u + " and v - " + v + " v.type = " + v.type + " symbolic - " + (v.substring("notamper_symbolic") != -1));
        if( n[1].type == STRING && (v.substring("notamper_symbolic") != -1))
        {
            n[0].notamper_symbolic = true; 
            n[1].notamper_symbolic = true; 
                    p( "\t\t updated n[0] - " + n[0] + " and n[1] - " + n[1]);
        }
        ...
        if (t) {
            switch (t) {
              ...
              case DIV:         
			  	v = u / v; 
				print (" performing division in assignment \n");
				break;
              case MOD:         v = u % v; break;
            }
        }
        p ( " \t \t \t put value arguments - r - " +  r + " v - " + v + " n[0] - " + n[0]);
        putValue(r, v, n[0]);
        p ( " \t \t \t n[0] after put value - " + n[0] + " \n");        
        break;
      ...
      case BITWISE_OR:
                    p ( " Execute : BITWISE_OR ");

        v = getValue(execute(n[0], x)) | getValue(execute(n[1], x));
        break;

      case BITWISE_XOR:
                    p ( " Execute :  BITWISE_XOR");
      ...
      case DIV:
        p( " div - numerator - " + n[0] + " denominator - " + n[1]);
        v = getValue(execute(n[0], x)) / getValue(execute(n[1], x));
        break;
      ...
      case LIST:
        ...
        for (i = 0, j = n.length; i < j; i++) {
            value = execute(n[i], x);
            u = getValue(value);
            u_symbolic = getValueSymbolic(value);
            p ( " \t defining property - " + u + " symbolic - " + u_symbolic);
            v.__defineProperty__(i, u, false, false, true);
            v_symbolic.__defineProperty__(i, u_symbolic, false, false, true);
        }
        v.__defineProperty__('length', i, false, false, true);
        v.__defineProperty__('notamper_symbolic', v_symbolic, false, false, true);
        break;

      case CALL:
          p ( " execute CALL "  + "n[0] - " + n[0] + " n[1] - " + n[1]);
        p('execute 1');
        r = execute(n[0], x);
        p('execute 2');
        a = execute(n[1], x);
        ...
        if (t instanceof Activation)
            t = null;
        p('calling the function - ' + t + " a - " + a + " x  - " + x); 
        v = f.__call__(t, a, x);
        break;
      ...
      case IDENTIFIER:
        p ( " in identifier - ");
        for (s = x.scope; s; s = s.parent) {
            p (" \t n.value = " + n.value + " and n.symbolic -  " + n.notamper_symbolic + " scope - " + s.object + " \n");
...
print (" from jsexec : calling the test-3.js");
evaluate(snarf("/home/prithvi/clean/prithvi_repo/papers/notamper/trunk/src/narcissus/mozilla-1.9.2/js/narcissus/test-3.js"), "/home/prithvi/clean/prithvi_repo/papers/notamper/trunk/src/narcissus/mozilla-1.9.2/js/narcissus/test-3.js", 1);
