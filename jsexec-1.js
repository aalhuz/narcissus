...
function getValue(v) {
    ...
    if (v instanceof Reference) {
        ...
        p ("getValue: return 1 - " + v.base[v.propertyName] + " property - " + v.propertyName  + " and symbolic - " + v.base["notamper_symbolic"]);
        return v.base[v.propertyName];
    }
    p (" getValue  return 2- " + v);
    return v;
}

function putValue(v, w, vn) {
        p(" putValue  called  on : " + v + " property name "+ v.propertyName + " w = " + w + " file - " + v.node.filename  + " line - " + v.node.lineno);
    ...
    if (v instanceof Reference)
    {
        ...
        {
            p ( " here ");
         (v.base || global) ['notamper_symbolic'] = true;
...
//    p ( " execute : called " + n + " and x - " + x );
    ...
    switch (n.type) {
      ...
      case IF:
        p(" If condition - " + n.condition + " then - " + n.thenPart + " else - " + n.elsePart + " execution context - " + x);
      ...
      case CALL:
          p ( " execute CALL "  + n[0].filename + " line - " + n[0].lineno);
        r = execute(n[0], x);
        a = execute(n[1], x);
        f = getValue(r);
